...
layui.define(['layer', 'form'], function (exports) {
    ...
    var $ = layui.$
        ...
        that.config = {
            //默认工具bar
            tool: [
                'strong', 'italic', 'underline', 'del', 'sup', 'sub'
                , '|'
                , 'left', 'center', 'right','blockquote'
                , '|'
    ...
    //插入内容
    Edit.prototype.insertContent = function (index, text) {
        var iframeWin = getWin(index);
        if (!iframeWin[0]) return;

        iframeWin[0].document.body.focus();
        iframeWin[0].document.execCommand('insertHTML',false,text);
        /*
        var range = Range(iframeWin[0].document), elem = document.createElement('inserted'), offset = 0;
        if (iframeWin[0].selection) { //IE
            var text = range.text || text;
            ...
            range.select();
            if (range.htmlText != undefined && range.htmlText != "") iframeWin[0].selection.clear();
            range.moveStart('character', -iframeWin[0].document.body.innerHTML.toString().length);
            offset = range.text.length + FilterHtmlTag(range.htmlText) + 1;
        } else { //非IE
        ...
        layui.layedit.setContent(index, layui.layedit.getContent(index).replace('<inserted>', '').replace('</inserted>', ''));
        range = Range(iframeWin[0].document);
        console.log(range);
    ...
    var setIframe = function (editor, textArea, set) {
            ...
            iframe.css({
                ...
                var conts = iframe.contents()
                    ...
                    , style = !!set.style ? set.style + defaultCSS : defaultCSS
        ...
        , Range = function (iframeDOM) {
            $(iframeDOM.body).focus();
            var selection, range;
            if (iframeDOM.selection) {
                selection = iframeDOM.selection;
                range = selection.createRange();
            } else {
                selection = iframeDOM.getSelection();
                range = selection != undefined ? selection.getRangeAt(0) : iframeDOM.createRange();
            }
            return range;
        }
        ...
        , toolCheck = function (tools, othis) {
            ...
            $(container).parents().each(function () {
                ...
                if (tagName === 'blockquote') {
                    item('blockquote').addClass(CHECK)
                }
        ...
        , toolActive = function (iframeWin, editor, set) {
            var iframeDOM = iframeWin.document
                ...
                if (command) {
                    var addition = parseInt(othis.attr('lay-addition')) || othis.attr('lay-addition') || null;
                    console.log(command, addition);
                    if (!!addition) {
                        iframeDOM.execCommand(command, false, addition);
                    } else {
        ...
        , tools = {
            ...
            ,
            sup: '<i class="layui-icon layedit-tool-sup" title="上标" lay-command="superscript" layedit-event="sup">x<sup>2</sup></i>'
            ,
            sub: '<i class="layui-icon layedit-tool-sub" title="下标" lay-command="subscript" layedit-event="sub">x<sub>2</sub></i>'
            ,
            ...
            ,
            blockquote: '<i class="layui-icon layedit-tool-chat" title="引用" layedit-event="blockquote">&#xe606;</i>'
            ,
